...
### D6. 회복탄력성: 타임아웃·재시도를 게이트웨이가 집약

- 채택: 세 공급자 WebClient 가 공유하는 회복탄력성 정책(`ProviderWebClients`)을 조립 계층에 둔다. 정책은 `gateway.provider-*` 설정으로 외부화한다.
  - 타임아웃 3종: 연결(TCP 수립), 응답(요청 전송 후 헤더 수신), 읽기 유휴(헤더 수신 후 읽기 없는 최대 시간). 응답 타임아웃은 헤더 수신 시 해제되어 정상 장문 스트림을 끊지 않고, 읽기 유휴 타임아웃은 헤더 수신 후 스트림이 멎는 경우의 무한 대기를 차단한다.
  - 재시도: 일시적 오류(응답/유휴 타임아웃, 5xx, 연결 거부) 한정 지수 백오프. 4xx 는 재시도하지 않는다.
- 근거: 배경(문제)에서 든 "재시도가 호출부마다 흩어진다"를 실제로 해소한다. 정책이 한 곳에 있어야 통제·관측·변경이 가능하다.
- 스트리밍 재시도 경계(핵심 안전 설계): 재시도는 응답 바디 스트림이 시작되기 전 단계(`ExchangeFilterFunction`)에만 건다. 헤더를 수신한 뒤에는 재시도하지 않는다. 따라서 토큰이 한 조각이라도 다운스트림으로 방출된 후 발생한 오류는 구조적으로 재실행되지 않는다. 부분 방출 후 재실행에 의한 토큰 중복·오염을 원천 차단하기 위함이다.
- 트레이드오프: 헤더 수신 후의 실패(스트림 중단)는 재시도로 복구하지 않고 오류로 전파한다. 스트리밍의 정확성(중복 없음)을 가용성보다 우선한 결정이다.
- 제외: 스트림 전체를 재구독하는 순진한 `retryWhen`. 이미 방출된 토큰을 재실행해 중복을 유발한다.

### D7. 오류 매핑: 미등록 공급자는 404

- 채택: 배선되지 않은 공급자를 지정한 요청은 도메인 예외(`UnknownProviderException`)로 표현하고 인바운드 어댑터가 404 로 매핑한다. 응답에 활성 공급자 목록을 담아 진단을 돕는다.
- 근거: 원인은 클라이언트가 존재하지 않는 자원을 가리킨 것이므로 서버 내부 오류(500)가 아니다. 매핑되지 않은 `IllegalArgumentException` 이 500 으로 새어 나가지 않게 도메인 예외로 좁힌다.
...
- 업스트림 hang: 연결·응답·읽기 유휴 타임아웃(D6)이 무한 대기를 차단한다. 헤더 수신 전 일시적 실패는 한정 재시도로 흡수한다.
- 공급자 SSE 장애: 헤더 수신 전 실패는 재시도, 헤더 수신 후 스트림 중단은 재시도 없이 UseCase로 전파(부분 UsageRecord 확정 후 클라이언트에 오류 이벤트). 스트림 중복을 막기 위한 경계다(D6).
- 미등록 공급자 요청: 404(`unknown_provider`)로 매핑(D7). 공급자 업스트림 HTTP 오류는 502 로 실제 상태·바디와 함께 노출.
